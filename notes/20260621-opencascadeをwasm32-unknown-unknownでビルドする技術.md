...
3. wasm例外処理 (exnref EH) を有効化する （OCCTはC++例外を投げる）。なお `panic = "abort"` は既定が abort なので明示不要（付けても可）
...
ここで `Cargo.toml` に `panic = "abort"` を**あえて書いていない**のが目に付きますが、これは「付けると壊れるから避けている」のではなく、**`wasm32-unknown-unknown` では既定がすでに abort なので書く必要がない**、という意味です（後述の実測の通り、付けても問題なくビルドできます）。
...
- `wasm32-unknown-unknown` はもともとabortが既定なので、`panic = "abort"` を明示しなくてもRustはアンワインドしない＝ legacy-EH の landing pad を吐かない。これがOCCT側の exnref EH と素直に共存できる理由。
- だからこの設定は「書いても書かなくてもほぼ同じ」。Cargo.toml のコメントが言いたいのも「既定が abort なので明示不要」という点であって、書くと壊れるという話ではない。

### 実測: `panic = "abort"` あり/なしで比較してみた

「あえて付けない」が本当に必要な判断なのか気になったので、`[profile.release]` に `panic = "abort"` を足した版と元の版で、同じクロスイメージ（`make deploy-cross`、wasm-opt は `-all` で exnref を含む全機能有効）でビルドして比較しました。

| | `panic="abort"` なし（元） | `panic="abort"` あり |
|---|---|---|
| ビルド | ✅ 成功 | ✅ 成功 |
| wasm-opt `-all`（exnref）検証 | ✅ success | ✅ success |
| 最終 wasm サイズ | 14,286,121 B | 14,286,050 B（**−71 B**） |

結果は **どちらも成功**し、出力差はわずか **71バイト**でした。つまり `panic = "abort"` を付けても **exnref EH と衝突せず、ビルドは壊れません**。「既定が abort なので明示は不要（書いても実質ノーオペ）」というのが実態で、`panic` 設定がOCCTの例外処理を壊す要因にはなっていない、と確認できました。

> ⚠️ もともと記事では「あえて付けない＝付けると危険」というニュアンスで書いていましたが、上の実測の通りそれは言い過ぎでした。正確には「既定が abort なので付ける必要がないだけ」です。
...
3. exnref EHを有効化する（Rust側は既定が abort なので `panic = "abort"` の明示は不要。実測では付けてもビルドは通る）
